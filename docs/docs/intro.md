...
- **自动滚动** - 在 Reading 模式下，如果有打开的文件，自动滚动编辑器内容，模拟人类阅读行为
...
| `ghostReader.displayWidth`          | 状态栏每次显示的最大字符数（智能分页）      | 45     |
| `ghostReader.autoStopDelay`         | 停留在当前行多少秒后自动停止（0为永不停止） | 0      |
| `ghostReader.autoScrollInterval`    | 自动滚动间隔时间（毫秒），0 为禁用          | 300    |
| `ghostReader.autoScrollPauseDuration` | 自动滚动停顿时间（毫秒）                    | 5000   |
