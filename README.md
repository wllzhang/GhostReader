...
- 📜 **自动滚动** - 在 Reading 模式下自动滚动编辑器内容，模拟人类阅读行为
...
| `ghostReader.autoScrollInterval`    | 自动滚动间隔（毫秒）         | 300    | 0-5000    |
| `ghostReader.autoScrollPauseDuration` | 自动滚动停顿时间（毫秒）     | 5000   | 0-30000   |
...
> 
> 💡 **自动滚动**: 在 Reading 模式下，如果有打开的文件，会自动滚动编辑器内容。设置 `autoScrollInterval` 为 0 可禁用此功能。滚动过程中会随机停顿，停顿时间由 `autoScrollPauseDuration` 控制
